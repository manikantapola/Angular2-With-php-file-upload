...
export class Ng2UploaderService {
  ...
  uploadFilesInQueue(msg): void {
    let newFiles = this._queue.filter((f) => { return !f.uploading; });
    newFiles.forEach((f) => {
      this.uploadFile(f, msg);
      if(msg != ''){return;}
    });
  };

  uploadFile(file: any, msg: string): void {
  ...
  addFilesToQueue(files: File[], message:string): void {
    this.clearQueue();
    ...
    [].forEach.call(files, (file: File, i: number) => {
      if (!this.inQueue(file)) {
        ...
        }else if( (message != '' && i == 1) || (files.length == 1) ){
          this._queue.push(file);
        }
    ...
    if (this.opts.autoUpload) {
      this.uploadFilesInQueue(message);
    }
  }
